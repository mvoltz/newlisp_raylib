; see the excellent cheat sheet at: https://www.raylib.com/cheatsheet/cheatsheet.html

(context 'rl)										; separate raylib symbols from MAIN context symbols
(define (gf x) (import "libraylib.so" x))			; function to import raylib functions

; window-related functions

(gf "InitWindow")									; (rl:InitWindow 800 450 "Window Title") 
(gf "WindowShouldClose")							; check if KEY_ESCAPE or close icon pressed
(gf "CloseWindow")									; close the window
(gf "IsWindowReady")								; check if window is ready
(gf "IsWindowFullscreen")							; check if window is fullscreen
(gf "IsWindowHidden")								; check if window is hidden
(gf "IsWindowMinimized")							; check if window is currently minimize
(gf "IsWindowMaximized")							; check if window is currently maximized
(gf "IsWindowFocused")								; check if window is currently focused
(gf "IsWindowResized")								; check if window resized since last frame
(gf "IsWindowState")								; unsigned int - check if one specific window flag is enabled
(gf "SetWindowState")								; unsigned int - set window configuration state using flags
(gf "ClearWindowState")								; unsigned int - clear window configuration state flags
(gf "ToggleFullscreen")								; toggle fullscreen/windowed (only on PLATFORM_DESKTOP)
(gf "MaximizeWindow")								; set window state: maximized, if resizable (PLATFORM_DESKTOP)  
(gf "MinimizeWindow")								; set window state: minimized, if resizable (PLATFORM_DESKTOP)  
(gf "RestoreWindow")								; restore window state: minimized/maximized (PLATFORM_DESKTOP)
(gf "SetWindowIcon")								; set icon for window (PLATFORM_DESKTOP)
(gf "SetWindowTitle")								; set window title (PLATFORM_DESKTOP)
(gf "SetWindowPosition")							; set window position on screen (PLATFORM_DESKTOP)
(gf "SetWindowMonitor")								; set monitor for current window (fullscreen mode)
(gf "SetWindowMinSize")								; set minimum window dimensions
(gf "SetWindowSize")								; set window dimensions
(gf "GetWindowHandle")								; void - get native window handle
(gf "GetScreenWidth")                               ; Get current screen width
(gf "GetScreenHeight")                              ; Get current screen height
(gf "GetMonitorCount")                              ; Get number of connected monitors
(gf "GetCurrentMonitor")                            ; Get current connected monitor
(gf "GetMonitorPosition")                           ; Vector2 - Get specified monitor position
(gf "GetMonitorWidth")                              ; Get specified monitor width (max available by monitor)
(gf "GetMonitorHeight")                             ; Get specified monitor height (max available by monitor)
(gf "GetMonitorPhysicalWidth")                      ; Get specified monitor physical width in millimetres
(gf "GetMonitorPhysicalHeight")                     ; Get specified monitor physical height in millimetres
